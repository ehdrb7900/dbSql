-- 복습 where 11
-- job이 SALESMAN이거나 입사일자가 1981년 6월 1일 이후인 직원의 정보를 조회
SELECT *
FROM emp
WHERE job = 'SALESMAN'
...
-- 페이징 처리를 위한 꼼수 ROWNUM에 별칭을 부여, 해당SQL을 INLINE VIEW로 감싸고
-- 별칭을 통해 페이징 처리
...
-- LPAD : 문자열에 특정 문자열을 삽입SELECT CONCAT('HELLO', ', WORLD') CONCAT,
...
-- 위 4개의 컬럼을 생성하여 조회하는 쿼리 작성
SELECT TO_DATE('2019/12/31', 'YYYY/MM/DD') "12월 31일", TO_DATE('2019/12/31', 'YYYY/MM/DD') - 5 "12월 31일 - 5일", 
        SYSDATE "현재 날짜", SYSDATE - 3 "현재 날짜 -3일" 
FROM dual;

-- date format
-- 년도 : YYYY, YY, RR : 두 자리일때랑 4자리 일때랑 다름
-- RR : 50보다 클 경우 앞자리는 19, 작을 경우 앞자리는 20
